function table(rootEl){
    ...
    this.updateIds = function(){this.getJson().forEach((each,i)=>each.id = i + 1)}
    ...
    this.delete = function(id){
        this.json = this.json.filter(each=>each.id != id)
        this.oldJson = this.oldJson.filter(each=>each.id != id)
